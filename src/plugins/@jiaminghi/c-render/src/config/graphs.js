...
export const ellipse = {
    ...
    hoverCheck(position, { shape }) {
        ...
        const distance =
            getTwoPointDistance(position, leftFocusPoint) +
            getTwoPointDistance(position, rightFocusPoint)
...
export const regPolygon = {
    ...
    move({ movementX, movementY }, { shape, cache }) {
        ...
        cache.points = cache.points.map(([x, y]) => [
            x + movementX,
            y + movementY
        ])
    }
...
export const polyline = {
    ...
    move({ movementX, movementY }, { shape }) {
        ...
        const moveAfterPoints = points.map(([x, y]) => [
            x + movementX,
            y + movementY
        ])
...
export const smoothline = {
    ...
    move({ movementX, movementY }, { shape, cache }) {
        ...
        cache.hoverPoints = cache.hoverPoints.map(([x, y]) => [
            x + movementX,
            y + movementY
        ])
...
export const text = {
    ...
    draw({ ctx }, { shape }) {
        ...
        position = new Array(rowNum)
            .fill(0)
            .map((foo, i) => [x, y + i * lineHeight - offset])
