...
async function start() {
    ...
    const browserifyMap = await exec(
        'browserify build/entry.js > dist/transition.map.js --debug'
    )
    ...
    const browserifyMin = await exec(
        'browserify build/entry.js > dist/transition.min.js'
    )
    ...
    const uglifyjs = await exec(
        'uglifyjs dist/transition.min.js -o dist/transition.min.js'
    )
