...
async function start() {
    ...
    const browserifyMap = await exec(
        'browserify build/entry.js > dist/bezierCurve.map.js --debug'
    )
    ...
    const browserifyMin = await exec(
        'browserify build/entry.js > dist/bezierCurve.min.js'
    )
    ...
    const uglifyjs = await exec(
        'uglifyjs dist/bezierCurve.min.js -o dist/bezierCurve.min.js'
    )
