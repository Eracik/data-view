...
function getLinearGradientColor(ctx, begin, end, color) {
  ...
  var linearGradientColor = ctx.createLinearGradient.apply(
    ctx,
    (0, _toConsumableArray2['default'])(begin).concat(
      (0, _toConsumableArray2['default'])(end)
    )
  )
...
function getPolylineLength(points) {
  var lineSegments = new Array(points.length - 1)
    .fill(0)
    .map(function (foo, i) {
      return [points[i], points[i + 1]]
    })
  var lengths = lineSegments.map(function (item) {
    return getTwoPointDistance.apply(
      void 0,
      (0, _toConsumableArray2['default'])(item)
    )
  })
