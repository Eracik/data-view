...
function calcPiesRadius(pies, chart) {
    var maxRadius =
        Math.min.apply(
            Math,
            (0, _toConsumableArray2['default'])(chart.render.area)
        ) / 2
    pies.forEach(function (pie) {
...
function getRoseIncrement(pie) {
    ...
    if (roseIncrement === 'auto') {
        ...
        var allRadius = data.reduce(function (all, _ref4) {
            ...
            return [].concat(
                (0, _toConsumableArray2['default'])(all),
                (0, _toConsumableArray2['default'])(radius)
            )
        }, [])
        var minRadius = Math.min.apply(
            Math,
            (0, _toConsumableArray2['default'])(allRadius)
        )
...
function calcPiesPercent(pies) {
    pies.forEach(function (pie) {
        ...
        data.forEach(function (item) {
            ...
            item.percentForLabel = toFixedNoCeil(
                (value / sum) * 100,
                percentToFixed
            )
        })
        var percentSumNoLast = (0, _util2.mulAdd)(
            data.slice(0, -1).map(function (_ref5) {
                var percent = _ref5.percent
                return percent
            })
        )
        data.slice(-1)[0].percent = 100 - percentSumNoLast
        data.slice(-1)[0].percentForLabel = toFixedNoCeil(
            100 - percentSumNoLast,
            percentToFixed
        )
    })
    return pies
    ...
    pies.forEach(function (pie) {
        ...
        data.forEach(function (item, i) {
            var _getDataAngle = getDataAngle(data, i),
                _getDataAngle2 = (0, _slicedToArray2['default'])(
                    _getDataAngle,
                    2
                ),
...
function getDataAngle(data, i) {
    ...
    var percentSum = (0, _util2.mulAdd)(
        needAddData.map(function (_ref7) {
...
function getPieInsideLabelPos(pieItem, dataItem) {
    ...
    return _util.getCircleRadianPoint.apply(
        void 0,
        (0, _toConsumableArray2['default'])(center).concat([radius, angle])
    )
}
...
function calcPiesEdgeCenterPos(pies) {
    pies.forEach(function (pie) {
        ...
        data.forEach(function (item) {
            ...
            var pos = _util.getCircleRadianPoint.apply(
                void 0,
                (0, _toConsumableArray2['default'])(center).concat([
                    radius[1],
                    centerAngle
                ])
            )
...
function sortPiesFromTopToBottom(dataItem) {
    dataItem.sort(function (_ref10, _ref11) {
        var _ref10$edgeCenterPos = (0, _slicedToArray2['default'])(
                _ref10.edgeCenterPos,
                2
            ),
            t = _ref10$edgeCenterPos[0],
            ay = _ref10$edgeCenterPos[1]

        var _ref11$edgeCenterPos = (0, _slicedToArray2['default'])(
                _ref11.edgeCenterPos,
                2
            ),
...
function addLabelLineAndAlign(dataItem, pieItem) {
    ...
    dataItem.forEach(function (item) {
        ...
        var bendPoint = _util.getCircleRadianPoint.apply(
            void 0,
            (0, _toConsumableArray2['default'])(center).concat([radius, angle])
        )
