...
function legend(chart) {
    ...
    if (legend) {
        legend = (0, _util2.deepMerge)(
            (0, _util.deepClone)(_config.legendConfig, true),
            legend
        )
...
function calcLegendTextWidth(legend, chart) {
    ...
    data.forEach(function (item) {
        var status = item.status,
            name = item.name
        item.textWidth = getTextWidth(
            ctx,
            name,
            status ? textStyle : textUnselectedStyle
        )
    })
    return legend
}
...
function calcHorizontalPosition(legend, chart) {
    ...
    lines.forEach(function (line, i) {
        return line.forEach(function (item) {
            ...
            item.iconPosition = mergeOffset(iconPosition, [
                xOffset,
                realYOffset
            ])
            item.textPosition = mergeOffset(textPosition, [
                xOffset,
                realYOffset
            ])
            item.align = align
...
function calcVerticalPosition(legend, chart) {
    var _getVerticalXOffset = getVerticalXOffset(legend, chart),
        _getVerticalXOffset2 = (0, _slicedToArray2['default'])(
            _getVerticalXOffset,
            2
        ),
...
function getIconConfig(legendItem, updater) {
    ...
    return data.map(function (item, i) {
        return (0, _defineProperty2['default'])(
            ...
            'click',
            createClickCallBack(legendItem, i, updater)
        )
    })
}
...
function getIconShape(legendItem, i) {
    ...
    var _data$i$iconPosition = (0, _slicedToArray2['default'])(
            data[i].iconPosition,
            2
        ),
...
function getTextHoverRect(legendItem, i) {
    ...
    var _legendItem$data$i3 = legendItem.data[i],
        ...
        _legendItem$data$i3$t = (0, _slicedToArray2['default'])(
            _legendItem$data$i3.textPosition,
            2
        ),
