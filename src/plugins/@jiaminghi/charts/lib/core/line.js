...
function getLineFillBottomPos(lineAxis) {
    ...
    if (minValue < 0 && maxValue > 0) {
        var valueMinus = maxValue - minValue
        var posMinus = Math.abs(
            linePosition[0][changeIndex] - linePosition[1][changeIndex]
        )
...
function getGradientParams(lineItem) {
    ...
    var maxPos = Math.max.apply(
        Math,
        (0, _toConsumableArray2['default'])(mainPos)
    )
    var minPos = Math.min.apply(
        Math,
        (0, _toConsumableArray2['default'])(mainPos)
    )
    var beginPos = maxPos
...
function lineAreaDrawed(_ref5, _ref6) {
    ...
    var changeIndex = lineFillBottomPos.changeIndex,
        changeValue = lineFillBottomPos.changeValue
    var linePoint1 = (0, _toConsumableArray2['default'])(
...
function beforeChangeLineAndArea(graph, config) {
    ...
    if (pointsNum > graphPointsNum) {
        ...
        var newAddPoints = new Array(pointsNum - graphPointsNum)
            .fill(0)
            .map(function (foo) {
                return (0, _toConsumableArray2['default'])(lastPoint)
            })
        graphPoints.push.apply(
            graphPoints,
            (0, _toConsumableArray2['default'])(newAddPoints)
        )
...
function getStartLineConfig(lineItem) {
    ...
    if (lineDash) {
        ...
        realLineDash = (0, _toConsumableArray2['default'])(
            realLineDash
        ).reverse()
    }
