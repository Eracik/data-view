...
import { text } from '@/plugins/@jiaminghi/c-render/lib/config/graphs'

import {
  getCircleRadianPoint,
  checkPointIsInRect
} from '@/plugins/@jiaminghi/c-render/lib/plugin/util'

import { getColorFromRgbValue } from '@/plugins/@jiaminghi/color'
...
const pie = {
  ...
  draw({ ctx }, { shape }) {
    ...
    const connectPoint1 = getCircleRadianPoint(rx, ry, or, endAngle).map(
      (p) => parseInt(p) + 0.5
    )
    const connectPoint2 = getCircleRadianPoint(rx, ry, ir, startAngle).map(
      (p) => parseInt(p) + 0.5
    )
...
const agArc = {
  ...
  draw({ ctx }, { shape, style }) {
    ...
    for (let i = 0; i < gradientArcNum; i++) {
      ...
      const startPoint = getCircleRadianPoint(
        rx,
        ry,
        r,
        startAngle + angleGap * i
      )
      const endPoint = getCircleRadianPoint(
        rx,
        ry,
        r,
      ...
      const color = getLinearGradientColor(ctx, startPoint, endPoint, [
        gradient[i],
        gradient[i + 1]
      ])
