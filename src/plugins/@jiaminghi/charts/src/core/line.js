...
function getLineAxis(line, axisData) {
    ...
    const xAxis = axisData.find(
        ({ axis, index }) => axis === 'x' && index === xAxisIndex
    )
    const yAxis = axisData.find(
        ({ axis, index }) => axis === 'y' && index === yAxisIndex
    )
...
function getLineAreaConfig(lineItem) {
    const { animationCurve, animationFrame, lineFillBottomPos, rLevel } =
        lineItem
...
function formatterLabel(lineItem) {
    ...
    if (type === 'function')
        return data.map((value, index) => formatter({ value, index }))
