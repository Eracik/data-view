...
import { getCircleRadianPoint } from '@/plugins/@jiaminghi/c-render/lib/plugin/util'

import { deepMerge, initNeedSeries, radianToAngle } from '../util'

import { getRgbaValue } from '@/plugins/@jiaminghi/color'
...
function calcGaugesAxisTickPosition(gauges, chart) {
  gauges.forEach((gaugeItem) => {
    ...
    gaugeItem.tickPosition = new Array(splitNum).fill(0).map((foo, i) => {
...
function calcGaugesLabelPositionAndAlign(gauges, chart) {
  gauges.forEach((gaugeItem) => {
    ...
    const position = tickAngles.map((angle, i) =>
...
function calcGaugesLabelData(gauges, chart) {
  gauges.forEach((gaugeItem) => {
    ...
    const value = new Array(splitNum)
      .fill(0)
      .map((foo, i) => parseInt(min + valueGap * i))
...
function beforeChangeArc(graph, config) {
  ...
  if (cacheNum > needNum) {
    ...
    graphGradient.push(
      ...new Array(needNum - cacheNum).fill(0).map((foo) => [...last])
    )
  }
}
...
function getPointerStyle(gaugeItem) {
  ...
  return deepMerge(
    { rotate: radianToAngle(angle), scale: [1, 1], graphCenter: center },
    style
  )
}
