...
function getSeriesMinMaxValue(series) {
  ...
  const minValue = Math.min(
    ...series.map(({ data }) => Math.min(...filterNonNumber(data)))
  )

  const maxValue = Math.max(
    ...series.map(({ data }) => Math.max(...filterNonNumber(data)))
  )
...
function getAfterFormatterLabel(label, formatter) {
  ...
  if (typeof formatter === 'string')
    label = label.map((l) => formatter.replace('{value}', l))
  if (typeof formatter === 'function')
    label = label.map((value, index) => formatter({ value, index }))
...
function calcAxisTickPosition(allAxis, chart) {
  return allAxis.map((axisItem) => {
    ...
    const tickPosition = new Array(labelNum).fill(0).map((foo, i) => {
      if (axis === 'x') {
    ...
    const tickLinePosition = getTickLinePosition(
      axis,
      boundaryGap,
      position,
      tickPosition,
