...
function setBarAxis(bars, chart) {
    ...
    bars.forEach((bar) => {
        ...
        const xAxis = axisData.find(
            ({ axis, index }) => `${axis}${index}` === `x${xAxisIndex}`
        )
        const yAxis = axisData.find(
            ({ axis, index }) => `${axis}${index}` === `y${yAxisIndex}`
        )
...
function groupBarByLabelAxis(bars) {
    ...
    return labelAxis.map((axisIndex) => {
        return bars.filter(
            ({ labelAxis: { axis, index } }) => axis + index === axisIndex
        )
    })
}
...
function setBarWidthAndGap(bars) {
    ...
    if (typeof barWidth === 'number' || barWidth !== 'auto') {
        widthAndGap = getBarWidthAndGapWithPercentOrNumber(
            barCategoryWidth,
            barWidth,
            barGap,
            barNum
        ...
        widthAndGap = getBarWidthAndGapWidthAuto(
            barCategoryWidth,
            barWidth,
            barGap,
            barNum
...
function calcBarLabelAxisCoordinate(bars) {
    return bars.map((bar) => {
        ...
        const barLabelAxisPos = tickPosition.map((tick, i) => {
            const barCategoryStartPos =
                tickPosition[i][coordinateIndex] - tickGap / 2

            const barItemsStartPos =
                barCategoryStartPos + (tickGap - barAllWidthAndGap) / 2

            return (
                barItemsStartPos +
                (barIndex + 0.5) * barWidth +
                barIndex * barGap
            )
        })
...
function calcBarValueAxisCoordinate(bars) {
    return bars.map((bar) => {
        ...
        const endPos = data.map((v) =>
            getValuePos(minValue, maxValue, v, linePosition, axis)
        )
...
function getValuePos(min, max, value, linePosition, axis) {
    ...
    const posMinus =
        linePosition[1][coordinateIndex] - linePosition[0][coordinateIndex]
...
function getLeftEchelonShape(barItem, i) {
    const { barValueAxisPos, barLabelAxisPos, barWidth, echelonOffset } =
        barItem
...
function getBarStyle(barItem, i) {
    const { barStyle, gradient, color, independentColor, independentColors } =
        barItem
...
function getFormatterLabels(barItem) {
    ...
    if (type === 'string')
        return data.map((d) => formatter.replace('{value}', d))

    if (type === 'function')
        return data.map((d, i) => formatter({ value: d, index: i }))
