...
function getRoseIncrement(pie) {
    ...
    if (roseIncrement === 'auto') {
        ...
        const allRadius = data.reduce(
            (all, { radius }) => [...all, ...radius],
            []
        )
...
function calcPiesPercent(pies) {
    pies.forEach((pie) => {
        ...
        const percentSumNoLast = mulAdd(
            data.slice(0, -1).map(({ percent }) => percent)
        )
...
function getOutsideLabelLineStyle(pieItem, i) {
    ...
    return deepMerge(
        { stroke: color, lineDash: [data[i].labelLineLength, 0] },
        labelLineStyle
    )
}
