...
function calcRadarAxisRingRadius(radarAxis, chart) {
    ...
    radarAxis.ringRadius = new Array(splitNum)
        .fill(0)
...
function calcRadarAxisLinePosition(radarAxis) {
    ...
    const angles = new Array(indicatorNum)
        .fill(0)
...
function getSplitAreaConfig(radarAxis) {
    const { areaRadius, polygon, animationCurve, animationFrame, rLevel } =
        radarAxis
...
function getSplitAreaStyle(radarAxis, i) {
    const { splitArea, ringRadius, axisLineAngles, polygon, centerPos } =
        radarAxis
    ...
    if (polygon) {
        const point1 = getCircleRadianPoint(
            ...centerPos,
            ringRadius[0],
            axisLineAngles[0]
        )
        const point2 = getCircleRadianPoint(
            ...centerPos,
            ringRadius[0],
            axisLineAngles[1]
        )
...
function getSplitLineConfig(radarAxis) {
    const { ringRadius, polygon, animationCurve, animationFrame, rLevel } =
        radarAxis
...
function getAxisLineConfig(radarAxis) {
    const { axisLinePosition, animationCurve, animationFrame, rLevel } =
        radarAxis
...
function getAxisLabelConfig(radarAxis) {
    const { axisLabelPosition, animationCurve, animationFrame, rLevel } =
        radarAxis
