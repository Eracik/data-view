...
function calcHorizontalPosition(legend, chart) {
    ...
    const xOffsets = lines.map((line) =>
        getHorizontalXOffset(line, legend, chart)
    )
...
function getBeforeWidth(startIndex, currentIndex, legend) {
    ...
    return (
        mulAdd(beforeItem.map(({ textWidth }) => textWidth)) +
...
function getHorizontalXOffset(data, legend, chart) {
    ...
    const allWidth =
        mulAdd(data.map(({ textWidth }) => textWidth)) +
        dataNum * (5 + iconWidth) +
        (dataNum - 1) * itemGap
...
function getIconConfig(legendItem, updater) {
    const { data, selectAble, animationCurve, animationFrame, rLevel } =
        legendItem
